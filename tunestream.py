...
async def build_autodj_query(cur, guild_id):
    await cur.execute("SELECT title FROM tunestream_history WHERE guild_id = %s ORDER BY played_at DESC LIMIT 8", (guild_id,))
...
async def maybe_enqueue_autodj(cur, guild, channel_id):
    ...
    try:
        ...
        history_titles = set()
        await cur.execute("SELECT title FROM tunestream_history WHERE guild_id = %s ORDER BY played_at DESC LIMIT 12", (guild.id,))
...
async def restore_active_playback_entry(cur, guild_id, requester_id=None):
    await cur.execute(
        "SELECT channel_id, video_url, title, position_seconds FROM tunestream_playback_state WHERE guild_id = %s AND bot_name = %s AND is_playing = TRUE LIMIT 1",
        (guild_id, 'tunestream')
    )
    row = await cur.fetchone()
    if not row:
    ...
    await cur.execute(
        "SELECT id, video_url FROM tunestream_queue WHERE guild_id = %s AND bot_name = %s ORDER BY id ASC LIMIT 1",
        (guild_id, 'tunestream')
    )
...
async def bootstrap_recovery_from_storage():
    ...
    try:
        async with DBPoolManager() as pool:
            async with pool.acquire() as conn:
                async with conn.cursor(aiomysql.DictCursor) as cur:
                    await cur.execute("SELECT guild_id, channel_id, position_seconds FROM tunestream_playback_state WHERE is_playing = TRUE AND bot_name = %s", ('tunestream',))
                    rows = await cur.fetchall()
    except Exception as exc:
            ...
            async with conn.cursor() as cur:
                for guild_id, data in list(playback_tracking.items()):
                    ...
                    if guild and guild.voice_client and (getattr(guild.voice_client, 'playing', False) or getattr(guild.voice_client, 'paused', False)):
                        ...
                        await cur.execute("REPLACE INTO tunestream_playback_state (guild_id, bot_name, channel_id, video_url, position_seconds, is_playing, title) VALUES (%s, 'tunestream', %s, %s, %s, %s, %s)", (guild_id, data['channel_id'], data['url'], pos, is_playing, data['title']))
...
@bot.tree.command(name="tunestream_main_setfeedback", description="Choose the text channel where this bot posts now-playing updates, queue actions, and recovery notices.")
@commands.has_permissions(administrator=True)
...
# --- PLAYBACK COMMANDS ---
@bot.tree.command(name="tunestream_main_play", description="Queue a track, URL, livestream, search result, or supported playlist and start playback if the bot is idle.")
...
async def aria_command_listener():
    try:
        ...
        for row in commands:
            ...
            cmd = row['command']
            if cmd == 'RESTART':
                logger.warning('[tunestream] Restart command received from swarm override listener. Restarting process...')
                async with DBPoolManager() as pool:
                    async with pool.acquire() as conn:
                        async with conn.cursor() as cur:
                            await cur.execute("DELETE FROM tunestream_swarm_overrides WHERE bot_name = %s", ('tunestream',))
                await bot.close()
                sys.exit(0)
            guild = bot.get_guild(guild_id)
