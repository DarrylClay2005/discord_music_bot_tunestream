...
DB_CONFIG = {
    ...
    'db': os.getenv(f"{BOT_ENV_PREFIX}_DB_NAME", "discord_music_tunestream"),
    'autocommit': True
}
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                await cur.execute("CREATE TABLE IF NOT EXISTS tunestream_playback_state (guild_id BIGINT, bot_name VARCHAR(50), channel_id BIGINT, video_url TEXT, position_seconds INT DEFAULT 0, is_playing BOOLEAN DEFAULT FALSE, title TEXT, PRIMARY KEY (guild_id, bot_name))")
                ...
                async with DBPoolManager() as pool:
                    async with pool.acquire() as conn:
                        async with conn.cursor() as cur:
                            ...
                            if reason == "FINISHED":
                                if loop_mode == 'queue':
                                    await cur.execute("INSERT INTO tunestream_queue (guild_id, bot_name, video_url, title, requester_id) VALUES (%s, 'tunestream', %s, %s, %s)", (payload.player.guild.id, payload.track.uri, payload.track.title, bot.user.id if bot.user else None))
                                elif loop_mode == 'song':
                                    await cur.execute("SELECT COALESCE(MIN(id), 0) FROM tunestream_queue WHERE guild_id = %s", (payload.player.guild.id,))
                                    min_row = await cur.fetchone()
                                    new_id = ((min_row[0] if min_row else 0) or 0) - 1
                                    await cur.execute("INSERT INTO tunestream_queue (id, guild_id, bot_name, video_url, title, requester_id) VALUES (%s, %s, 'tunestream', %s, %s, %s)", (new_id, payload.player.guild.id, payload.track.uri, payload.track.title, bot.user.id if bot.user else None))
        except Exception:
            pass
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                if not next_song:
                    await cur.execute("UPDATE tunestream_playback_state SET is_playing = FALSE WHERE guild_id = %s AND bot_name = 'tunestream'", (guild.id,))
                            ...
                            async with dj_pool.acquire() as dj_conn:
                                async with dj_conn.cursor(aiomysql.DictCursor) as dj_cur:
                                    ...
                                    if dj_res and dj_res.get('auto_dj'):
                                        ...
                                        await dj_cur.execute("INSERT INTO tunestream_queue (guild_id, bot_name, video_url, title, requester_id) VALUES (%s, 'tunestream', %s, %s, %s)", (guild.id, f"ytsearch:{genre} track", f"📻 Auto-DJ: {genre}", bot.user.id))
                ...
                await cur.execute("DELETE FROM tunestream_queue WHERE id = %s", (song_id,))
            ...
            async with pool.acquire() as conn:
                async with conn.cursor(aiomysql.DictCursor) as cur:
                    ...
                    for orphan in orphans:
                        ...
                        if guild:
                            await cur.execute("SELECT MIN(id) as m_id FROM tunestream_queue WHERE guild_id = %s", (guild.id,))
                            min_res = await cur.fetchone()
                            new_id = (min_res['m_id'] or 0) - 1
                            await cur.execute("INSERT INTO tunestream_queue (id, guild_id, bot_name, video_url, title, requester_id) VALUES (%s, %s, 'tunestream', %s, %s, %s)", (new_id, guild.id, orphan['video_url'], orphan.get('title', 'Resumed Track'), bot.user.id))
            ...
            async with conn.cursor() as cur:
                for guild_id, data in list(playback_tracking.items()):
                    ...
                    if guild and guild.voice_client and getattr(guild.voice_client, 'playing', False):
                        ...
                        await cur.execute("REPLACE INTO tunestream_playback_state (guild_id, bot_name, channel_id, video_url, position_seconds, is_playing, title) VALUES (%s, 'tunestream', %s, %s, %s, TRUE, %s)", (guild_id, data['channel_id'], data['url'], pos, data['title']))

# --- SETTINGS COMMANDS ---
@bot.tree.command(name="tunestream_main_sethome", description="Set bot's default voice/stage channel")
...
@bot.tree.command(name="tunestream_main_setfeedback", description="Set the text channel for bot announcements")
@commands.has_permissions(administrator=True)
...
@bot.tree.command(name="tunestream_main_djrole", description="Set DJ Role (Admins)")
@commands.has_permissions(administrator=True)
...
@bot.tree.command(name="tunestream_main_removedj", description="Remove DJ Role")
@commands.has_permissions(administrator=True)
...
@bot.tree.command(name="tunestream_main_djmode", description="Toggle Strict DJ Mode")
@commands.has_permissions(administrator=True)
...
@bot.tree.command(name="tunestream_main_247", description="Toggle 24/7 Mode")
@commands.has_permissions(administrator=True)
...
@bot.tree.command(name="tunestream_main_restart", description="Forcefully restart the bot instance (Admins only)")
@commands.has_permissions(administrator=True)
...
# --- PLAYBACK COMMANDS ---
@bot.tree.command(name="tunestream_main_play", description="Play a song, link, livestream, or YouTube playlist")
        ...
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                for track in entries_to_add:
                    await cur.execute("INSERT INTO tunestream_queue (guild_id, bot_name, video_url, title, requester_id) VALUES (%s, 'tunestream', %s, %s, %s)", (interaction.guild.id, track.uri, track.title, interaction.user.id))
                    added_count += 1
...
@bot.tree.command(name="tunestream_main_playnext", description="Put song at top of queue")
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                await cur.execute("INSERT INTO tunestream_queue (guild_id, bot_name, video_url, title, requester_id) VALUES (%s, 'tunestream', %s, %s, %s)", (interaction.guild.id, track.uri, track.title, interaction.user.id))
                for r in q: await cur.execute("INSERT INTO tunestream_queue (guild_id, bot_name, video_url, title, requester_id) VALUES (%s, 'tunestream', %s, %s, %s)", (interaction.guild.id, r[1], r[2], r[3]))
    await interaction.followup.send(embed=discord.Embed(description=f"**Playing next:** {track.title}", color=discord.Color.green()))

@bot.tree.command(name="tunestream_main_skip", description="Skip current song")
...
@bot.tree.command(name="tunestream_main_stop", description="Stop music and clear state")
...
@bot.tree.command(name="tunestream_main_pause", description="Pause playback")
...
@bot.tree.command(name="tunestream_main_resume", description="Resume playback")
...
@bot.tree.command(name="tunestream_main_clear", description="Clear the queue")
async def clear(interaction: discord.Interaction):
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                await cur.execute("DELETE FROM tunestream_queue WHERE guild_id = %s AND bot_name = 'tunestream'", (interaction.guild.id,))
                await cur.execute("UPDATE tunestream_playback_state SET is_playing = FALSE, position_seconds = 0 WHERE guild_id = %s AND bot_name = 'tunestream'", (interaction.guild.id,))
    await bot.change_presence(status=discord.Status.online)
...
@bot.tree.command(name="tunestream_main_join", description="Force bot to join your channel")
...
@bot.tree.command(name="tunestream_main_leave", description="Force bot to leave")
...
@bot.tree.command(name="tunestream_main_queue", description="View queue")
async def queue_cmd(interaction: discord.Interaction):
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                await cur.execute("SELECT title FROM tunestream_queue WHERE guild_id = %s AND bot_name = 'tunestream' ORDER BY id ASC LIMIT 10", (interaction.guild.id,))
...
@bot.tree.command(name="tunestream_main_shuffle", description="Shuffle the queue")
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                await cur.execute("DELETE FROM tunestream_queue WHERE guild_id = %s AND bot_name = 'tunestream'", (interaction.guild.id,))
                for row in l: await cur.execute("INSERT INTO tunestream_queue (guild_id, bot_name, video_url, title, requester_id) VALUES (%s, 'tunestream', %s, %s, %s)", (interaction.guild.id, row[1], row[2], row[3]))
...
@bot.tree.command(name="tunestream_main_remove", description="Remove song by queue number")
async def remove(interaction: discord.Interaction, index: int):
    if not await is_dj(interaction): return
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                await cur.execute("SELECT id FROM tunestream_queue WHERE guild_id = %s AND bot_name = 'tunestream' ORDER BY id ASC LIMIT 1 OFFSET %s", (interaction.guild.id, index-1))
                row = await cur.fetchone()
                if row: 
                    await cur.execute("DELETE FROM tunestream_queue WHERE id = %s", (row[0],))
...
@bot.tree.command(name="tunestream_main_skipto", description="Skip to a queue number")
async def skipto(interaction: discord.Interaction, index: int):
    if not await is_dj(interaction): return
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                await cur.execute("SELECT id FROM tunestream_queue WHERE guild_id = %s AND bot_name = 'tunestream' ORDER BY id ASC LIMIT %s", (interaction.guild.id, index-1))
                rows = await cur.fetchall()
                for r in rows: await cur.execute("DELETE FROM tunestream_queue WHERE id = %s", (r[0],))
...
@bot.tree.command(name="tunestream_main_move", description="Move song to new position")
...
# --- PLAYLISTS & HISTORY ---
@bot.tree.command(name="tunestream_main_savequeue", description="Save the current queue as a custom personal playlist")
...
@bot.tree.command(name="tunestream_main_loadqueue", description="Load a custom personal playlist into the current queue")
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                for url, title in q:
                    await cur.execute("INSERT INTO tunestream_queue (guild_id, bot_name, video_url, title, requester_id) VALUES (%s, 'tunestream', %s, %s, %s)", (interaction.guild.id, url, title, interaction.user.id))
...
@bot.tree.command(name="tunestream_main_leaderboard", description="Show the top 10 most played tracks in this server")
...
@bot.tree.command(name="tunestream_main_history", description="Show last 5 songs played in the server")
...
@bot.tree.command(name="tunestream_main_userhistory", description="See the last 10 tracks requested by a specific user")
...
@bot.tree.command(name="tunestream_main_steal", description="Steal a song from a user's history and add it to the queue")
...
@bot.tree.command(name="tunestream_main_grab", description="DM yourself the current song")
...
# --- MODIFIERS & FILTERS ---
@bot.tree.command(name="tunestream_main_volume", description="Set volume (1-200)")
async def volume(interaction: discord.Interaction, vol: int):
    if not await is_dj(interaction): return
    vol = max(1, min(200, vol))
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur: await cur.execute("INSERT INTO tunestream_guild_settings (guild_id, volume) VALUES (%s, %s) ON DUPLICATE KEY UPDATE volume = %s", (interaction.guild.id, vol, vol))
    if interaction.guild.voice_client:
...
@bot.tree.command(name="tunestream_main_loop", description="Toggle loop: off, song, queue")
async def loop_cmd(interaction: discord.Interaction, mode: str):
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur: await cur.execute("INSERT INTO tunestream_guild_settings (guild_id, loop_mode) VALUES (%s, %s) ON DUPLICATE KEY UPDATE loop_mode = %s", (interaction.guild.id, mode, mode))
...
@bot.tree.command(name="tunestream_main_filter", description="Apply an audio filter")
...
@bot.tree.command(name="tunestream_main_modify", description="Change speed/pitch")
async def modify_audio(interaction: discord.Interaction, speed: float = 1.0, pitch: float = 1.0, duration: int = 1):
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur: await cur.execute("UPDATE tunestream_guild_settings SET custom_speed = %s, custom_pitch = %s, custom_modifiers_left = %s WHERE guild_id = %s", (speed, pitch, duration, interaction.guild.id))
    if interaction.guild.voice_client:
        f = wavelink.Filters()
...
# --- SCRUBBING ---
@bot.tree.command(name="tunestream_main_seek", description="Seek to seconds")
...
@bot.tree.command(name="tunestream_main_replay", description="Restart current song")
...
# --- UTILITY & INFO ---
@bot.tree.command(name="tunestream_main_panel", description="Music control panel")
async def panel(interaction: discord.Interaction):
    ...
    await interaction.response.send_message(embed=discord.Embed(title="🎛️ Tunestream Control Panel", color=discord.Color.from_rgb(43, 45, 49)), view=P())

@bot.tree.command(name="tunestream_main_nowplaying", description="Show song status")
...
@bot.tree.command(name="tunestream_main_help", description="Command List")
async def help_cmd(interaction: discord.Interaction):
    cmds = [c.name for c in bot.tree.get_commands() if c.name.startswith("tunestream_main_")]
    await interaction.response.send_message(embed=discord.Embed(title="📚 Tunestream Commands", description=", ".join(cmds), color=discord.Color.blue()), ephemeral=True)
...
async def init_playlist_db():
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur: await cur.execute('''CREATE TABLE IF NOT EXISTS tunestream_active_playlists (guild_id BIGINT, bot_name VARCHAR(50), playlist_url TEXT, known_track_count INT DEFAULT 0, requester_id BIGINT, PRIMARY KEY (guild_id, bot_name))''')
    ...
    async with DBPoolManager() as pool:
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                await cur.execute("SELECT guild_id, playlist_url, known_track_count, requester_id FROM tunestream_active_playlists WHERE bot_name = 'tunestream'")
                    ...
                    async with pool.acquire() as conn:
                        async with conn.cursor() as cur:
                            for entry in new_tracks:
                                ...
                                if u: await cur.execute("INSERT INTO tunestream_queue (guild_id, bot_name, video_url, title, requester_id) VALUES (%s, 'tunestream', %s, %s, %s)", (gid, u, t, rid))
                            await cur.execute("UPDATE tunestream_active_playlists SET known_track_count = %s WHERE guild_id = %s", (cur_count, gid))
        ...
        async with DBPoolManager() as pool:
            async with pool.acquire() as conn:
                async with conn.cursor(aiomysql.DictCursor) as cur:
                    ...
                    await cur.execute("SELECT guild_id, command FROM tunestream_swarm_overrides WHERE bot_name = 'tunestream'")
        ...
        for row in commands:
            ...
            if guild and guild.voice_client:
                ...
                if cmd == 'STOP':
                    async with DBPoolManager() as pool:
                        async with pool.acquire() as conn:
                            async with conn.cursor() as cur: await cur.execute("DELETE FROM tunestream_queue WHERE guild_id = %s AND bot_name = 'tunestream'", (guild.id,))
                if exec: await send_webhook_log(bot.user.name, "🤖 Aria Override", f"Executed **{cmd}** in `{guild.name}`.", discord.Color.purple())
            async with DBPoolManager() as pool:
                async with pool.acquire() as conn:
                    async with conn.cursor() as cur: await cur.execute("DELETE FROM tunestream_swarm_overrides WHERE guild_id = %s AND bot_name = 'tunestream'", (row['guild_id'],))
    except: pass
        ...
        async with DBPoolManager() as pool:
            async with pool.acquire() as conn:
                async with conn.cursor(aiomysql.DictCursor) as cur:
                    ...
                    await cur.execute("SELECT * FROM tunestream_swarm_direct_orders WHERE bot_name = 'tunestream'")
                        ...
                        if tracks:
                            ...
                            async with DBPoolManager() as pool:
                                async with pool.acquire() as conn:
                                    async with conn.cursor() as cur:
                                        for t in es:
                                            await cur.execute("INSERT INTO tunestream_queue (guild_id, bot_name, video_url, title, requester_id) VALUES (%s, 'tunestream', %s, %s, %s)", (guild.id, t.uri, t.title, bot.user.id))
                                            count += 1
                    ...
                    if _has_human_listeners(guild.voice_client) and not force_leave:
                        ...
                        async with DBPoolManager() as pool:
                            async with pool.acquire() as conn:
                                async with conn.cursor() as cur:
                                    await cur.execute("DELETE FROM tunestream_queue WHERE guild_id = %s AND bot_name = 'tunestream'", (guild.id,))
                                    await cur.execute("REPLACE INTO tunestream_playback_state (guild_id, bot_name, is_playing) VALUES (%s, 'tunestream', FALSE)", (guild.id,))
                await send_webhook_log(bot.user.name, "🤖 Order Execution", f"Executed `{cmd}` order.", discord.Color.purple())
            async with DBPoolManager() as pool:
                async with pool.acquire() as conn:
                    async with conn.cursor() as cur: await cur.execute("DELETE FROM tunestream_swarm_direct_orders WHERE id = %s", (order['id'],))
    except: pass
...
class SwarmIntelligence(commands.Cog):
    def __init__(self, bot):
        self.bot, self.bot_name = bot, 'tunestream'
                ...
                if guild.voice_client and guild.voice_client.playing:
                    ...
                    if not track_title:
                        async with DBPoolManager() as pool:
                            async with pool.acquire() as conn:
                                async with conn.cursor(aiomysql.DictCursor) as cur:
                                    await cur.execute("SELECT title FROM tunestream_playback_state WHERE guild_id = %s AND bot_name = 'tunestream' AND is_playing = TRUE LIMIT 1", (guild.id,))
                                    res = await cur.fetchone()
                    ...
                    if guild.id in playback_tracking:
                        ...
                        if now - track_info.get('start_time', 0) > 10:
                            ...
                            async with DBPoolManager() as pool:
                                async with pool.acquire() as conn:
                                    async with conn.cursor() as cur:
                                        await cur.execute("SELECT id FROM tunestream_queue WHERE guild_id = %s LIMIT 1", (guild.id,))
