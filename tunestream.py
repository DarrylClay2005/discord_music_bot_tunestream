                ...
                async with DBPoolManager() as pool:
                    async with pool.acquire() as conn:
                        async with conn.cursor() as cur:
                            ...
                            if reason == "FINISHED":
                                if loop_mode == 'queue':
                                    await cur.execute("INSERT INTO tunestream_queue (guild_id, bot_name, video_url, title, requester_id) VALUES (%s, 'tunestream', %s, %s, %s)", (payload.player.guild.id, payload.track.uri, payload.track.title, original_requester))
                                elif loop_mode == 'song':
        ...
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                try:
                    ...
                    await insert_queue_front(cur, "tunestream_queue", guild.id, "tunestream", url, title, requester_id)
                ...
                async with conn.cursor(aiomysql.DictCursor) as cur:
                    ...
                    for orphan in orphans:
                        ...
                        if guild:
                            ...
                            await insert_queue_front(cur, "tunestream_queue", guild.id, "tunestream", orphan['video_url'], orphan.get('title', 'Resumed Track'), bot.user.id)
...
@bot.tree.command(name="tunestream_main_filter", description="Apply an audio filter to the music")
...
@bot.tree.command(name="tunestream_main_fade", description="Toggle 5-second smooth fade in/out transitions")
...
@bot.tree.command(name="tunestream_main_modify", description="Change speed and pitch for upcoming tracks")
...
@bot.tree.command(name="tunestream_main_forward", description="Skip forward X seconds")
...
@bot.tree.command(name="tunestream_main_rewind", description="Rewind X seconds")
...
# --- UTILITY & INFO ---
@bot.tree.command(name="tunestream_main_panel", description="Spawn the advanced music control panel")
            ...
            async with DBPoolManager() as pool:
                async with pool.acquire() as conn:
                    async with conn.cursor() as cur:
                        ...
                        for row in l: await cur.execute("INSERT INTO tunestream_queue (guild_id, bot_name, video_url, title, requester_id) VALUES (%s, 'tunestream', %s, %s, %s)", (i.guild.id, row[1], row[2], row[3]))
...
@bot.tree.command(name="tunestream_main_ping", description="Bot latency")
...
@bot.tree.command(name="tunestream_main_uptime", description="Bot uptime")
...
@bot.tree.command(name="tunestream_main_stats", description="Bot statistics")
...
@bot.tree.command(name="tunestream_main_help", description="List all TUNESTREAM commands")
                    ...
                    async with pool.acquire() as conn:
                        async with conn.cursor() as cur:
                            for entry in new_tracks:
                                ...
                                if t_url:
                                    await cur.execute("INSERT INTO tunestream_queue (guild_id, bot_name, video_url, title, requester_id) VALUES (%s, 'tunestream', %s, %s, %s)", (guild_id, t_url, t_title, req_id))
                                    added_count += 1
                            await cur.execute("UPDATE tunestream_active_playlists SET known_track_count = %s WHERE guild_id = %s", (current_count, guild_id))
        ...
        async with DBPoolManager() as pool:
            async with pool.acquire() as conn:
                async with conn.cursor(aiomysql.DictCursor) as cur:
                    ...
                    await cur.execute("SELECT guild_id, command FROM tunestream_swarm_overrides WHERE bot_name = %s", ('tunestream',))
            ...
            if guild and guild.voice_client:
                ...
                if cmd == 'STOP':
                    async with DBPoolManager() as pool:
                        async with pool.acquire() as conn:
                            async with conn.cursor() as cur:
                                await cur.execute("DELETE FROM tunestream_queue WHERE guild_id = %s AND bot_name = %s", (guild_id, 'tunestream'))
        ...
        async with DBPoolManager() as pool:
            async with pool.acquire() as conn:
                async with conn.cursor(aiomysql.DictCursor) as cur:
                    ...
                    await cur.execute("SELECT * FROM tunestream_swarm_direct_orders WHERE bot_name = %s", ('tunestream',))
                                ...
                                async with pool.acquire() as conn:
                                    async with conn.cursor() as cur:
                                        for track in entries:
                                            await cur.execute("INSERT INTO tunestream_queue (guild_id, bot_name, video_url, title, requester_id) VALUES (%s, %s, %s, %s, %s)", (guild.id, 'tunestream', track.uri, track.title, bot.user.id))
                                            added_count += 1
                            if playlist_url:
